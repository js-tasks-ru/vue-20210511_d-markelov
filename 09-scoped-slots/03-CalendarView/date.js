...
export function makeMonthDaysArr(inDate) {
  ...
  if (firstDay.getDay() != 1) {
    for (let i = 1; i < firstDay.getDay() && i < 7; i++) {
      daysArr.push({
        date: new Date(firstDay.getFullYear(), firstDay.getMonth(), firstDay.getDate() - i),
        inactive: true,
      });
    }
  }
  ...
  if (lastDay.getDay() != 0) {
    for (let i = 7; i > lastDay.getDay(); i--) {
      daysArr.push({
        date: new Date(lastDay.getFullYear(), lastDay.getMonth(), lastDay.getDate() + i - lastDay.getDay()),
        inactive: true,
      });
    }
  }
